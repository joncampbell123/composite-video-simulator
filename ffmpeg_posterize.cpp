...
class InputFile {
public:
    InputFile() : threshhold(3/*default*/) {
        input_avfmt = NULL;
        audio_dst_data = NULL;
...
static void help(const char *arg0) {
    ...
    fprintf(stderr," -threshhold <n>               Threshhold (8 for no effect, 1 for maximum posterization)\n");
}
...
void composite_layer(AVFrame *dstframe,AVFrame *srcframe,InputFile &inputfile) {
    ...
    shr = 8 - inputfile.threshhold;
    mask = ((0xFF << shr) & 0xFF) * 0x01010101UL;
