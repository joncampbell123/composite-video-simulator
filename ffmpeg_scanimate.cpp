...
void composite_layer(AVFrame *dstframe,AVFrame *srcframe,InputFile &inputfile,unsigned int field,unsigned long long fieldno) {
    ...
    // copy raster emu to screen
    for (y=field;y < dstframe->height;y++) {
        for (x=0;x < dstframe->width;x++) {
            rgba = rasteremu[(y*dstframe->width)+x] >> PRECISION;
            if (rgba > 255) rgba = 255;
            ((uint32_t*)(dstframe->data[0] + (dstframe->linesize[0] * y)))[x] = 0xFF000000UL + (rgba * 0x010101UL);
        }
    }
        ...
        do {
            ...
            while (current < upto) {
                ...
                for (std::vector<InputFile>::iterator i=input_files.begin();i!=input_files.end();i++) {
                    ...
                    composite_layer(output_avstream_video_frame,(*i).input_avstream_video_frame_rgb,*i,(current & 1) ^ 1,current);
                }
